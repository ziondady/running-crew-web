...
function RankingContent() {
  ...
  const handleDeleteLog = async (logId: number, deletedKm: number, ownerUserId: number) => {
    ...
    try {
      ...
      // 랭킹의 해당 유저 km 감산 (낙관적 업데이트)
      setRanking((prev) =>
        prev
          .map((r) => r.id === ownerUserId ? { ...r, km: Math.max(0, r.km - deletedKm) } : r)
          .sort((a, b) => b.km - a.km)
                            ...
                            {detailLoading ? (
                              ...
                              <div className="space-y-1">
                                {detailLogs.map((log: any) => {
                                  // 본인이거나 운영자(같은 크루 랭킹 안이므로 같은 크루)면 삭제 가능
                                  const canDelete = !!me && (me.id === r.id || me.role === "operator");
                                  return (
                                    <div key={log.id} className="flex items-center gap-2 text-[11px]">
                                      ...
                                      {canDelete && (
                                        <button
                                          onClick={(e) => { e.stopPropagation(); handleDeleteLog(log.id, log.buff_distance ?? 0, r.id); }}
